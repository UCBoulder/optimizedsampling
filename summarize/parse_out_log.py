...
def parse_path(root):
    """Parse directory path to extract experiment configuration."""
    ...
    if path_parts[dataset_idx + 1] == "empty_initial_set":
        ...
        # Find initial set (account for multiple/multiple_cluster_sampling)
...
def aggregate_results(base_dir=BASE_DIR, log_filename=LOG_FILENAME, dataset=None, 
    ...
    for root, _, files in os.walk(base_dir):
        ...
        if budget_list is not None and int(budget) not in budget_list:
            print(f"Skipping {key} because budget {budget} is not in {budget_list}")
            continue
        ...
        # Store updated metrics per method
        for metric in METRICS:
    ...
    for key, val in list(results.items()):
        ...
        for metric in METRICS:
            for method in list(val[f"{metric}_methods"].keys()):
                ...
                if not EXPECTED_SEEDS.issubset(seed_to_val.keys()):
                    missing = EXPECTED_SEEDS - seed_to_val.keys()
                    print(f"Skipping method {method} for {key}, {metric} due to missing seeds: {missing}")
                    continue
                ...
                # Compute delta (R2: updated-initial, others: initial-updated)
        ...
        # Remove experiment if no methods have complete data
        if not any(clean_methods.values()):
            print(f"Removing experiment for {key} - no complete methods")
            del results[key]
...
def build_filtered_df(results_dict, dataset, init_set, cost_type, alpha=None, 
    ...
    """Build DataFrame from results for specific configuration."""
    rows = []
    ...
    for (ds, iset, ctype, budget), data in results_dict.items():
        ...
        # Add updated metrics and deltas for each method
        for metric in METRICS:
        ...
        # Add "default" method using initial_metrics_lookup
...
def read_initial_metrics_from_json(dataset, initial_set, base_dir=BASE_DIR, 
    ...
    """Read initial metrics from JSON files for a given initial set.
    
    Returns: {metric: [values across seeds]}
    """
    # Build path to initial_set directory (outside cost_type folder)
    path_parts = [base_dir, dataset, model]
    if multiple:
        path_parts.append("multiple")
    ...
    if not os.path.exists(initial_set_dir):
        print(f"Initial set directory not found: {initial_set_dir}")
        return {}
    
    metrics_by_seed = {metric: {} for metric in METRICS}
    
    # Read JSON files for each seed
    for filename in os.listdir(initial_set_dir):
        ...
        # Extract seed from filename: initial_set_metrics_seed_{seed}.json
        ...
        try:
            ...
            # Extract metrics (JSON uses lowercase, we need initial_{metric} format)
            for metric in METRICS:
                if metric in data:
    ...
    # Convert to lists sorted by seed
    result = {}
    for metric in METRICS:
...
def collect_initial_metrics_for_dataset(dataset, initial_set_strs, PPC, NUM_STRATA, 
                                       multiple=False, expected_seeds=None):
    """Collect initial metrics for all initial sets in a dataset from JSON files.
    
    Returns: {(dataset, initial_set): {size: {metric: [values]}}}
    """
    ...
    for size in range(0, 5100, 50):
        ...
        try:
            ...
            print(f"Error collecting initial metrics for {dataset}, {initial_set}: {e}")
    
    return initial_metrics_by_set


def build_initial_metrics_lookup(initial_metrics_by_set):
    """Build lookup dictionary from collected initial metrics.
    
    Returns: {size: {metric_mean, metric_std, metric_se}}
    """
...
def process_dataset_configurations(dataset, initial_set_strs, PPC, NUM_STRATA, 
    ...
    """Process all configurations for a dataset and build dataframes.
    
    Returns: (dataset_dfs, multiple_dfs)
    """
    if alphas is None:
    ...
    for size in range(0, 5100, 100):
        ...
        for alpha in alphas:
            ...
            try:
                ...
                print(f"Error processing {dataset}, {initial_set}, {cost_type}: {e}")
        
        # Save combined results for this initial_set (non-multiple mode)
        if dataset_dfs and not multiple:
            ...
            print(f"Saved combined results for {dataset}, {initial_set}")
            dataset_dfs = []
...
def save_initial_metrics_summaries(dataset, initial_metrics_by_set, multiple=False):
    """Save initial metrics summary CSVs for a dataset."""
    for metric in METRICS:
        sizes = []
        means = []
        stds = []
        ...
        if len(sizes) > 0:
            ...
            os.makedirs(out_dir, exist_ok=True)
            summary_path = f"{out_dir}/{dataset}_initial_{metric}_summary.csv"
            summary_df.to_csv(summary_path, index=False)
            print(f"Saved initial {metric} summary to {summary_path}")
...
if __name__ == "__main__":
    ...
    for dataset in DATASET_NAMES:
        # Collect initial metrics for all initial sets from JSON files
        ...
        # Build lookup dictionary for use in dataframe construction
        initial_metrics_lookup = build_initial_metrics_lookup(initial_metrics_by_set)
        
        # Process all configurations and build dataframes
        ...
        # Save combined results for multiple mode
        ...
        # Save initial metrics summaries
