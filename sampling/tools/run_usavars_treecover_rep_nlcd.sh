...
run_setting3() {
    ...
    DATASET="usavars"
    ALPHAS=(10)
    GROUP_PATH="../../0_data/groups/usavars/treecover/NLCD_cluster_assignments_8_dict.pkl"
    GROUP_TYPE="nlcd"

    for SIZE in 100; do
        INIT_NAME_EXP="cluster_sampling_5_fixedstrata_10ppc_${SIZE}_size"
        INIT_NAME="cluster_sampling/5_fixedstrata_10ppc_${SIZE}_size"
        INIT_SET_IDS="../../0_data/initial_samples/usavars/treecover/cluster_sampling/fixedstrata_Alabama_01-Colorado_08-Montana_30-New York_36-Ohio_39/sample_state_combined_county_id_10ppc_${SIZE}_size_seed_${seed}.pkl"
                    ...
                    for BUDGET in "${BUDGETS[@]}"; do
                        for UTIL_LAMBDA in "${UTIL_LAMBDAS[@]}"; do
                            ...
                            if [ "$COST_FN" == "cluster_based" ]; then
                                UNIT_ASSIGNMENT_PATH="../../0_data/groups/usavars/treecover/county_assignments_dict.pkl"
                                UNIT_TYPE="cluster"
                                POINTS_PER_UNIT=10
                                ...
                                REGION_ASSIGNMENT_PATH="../../0_data/groups/usavars/treecover/state_assignments_dict.pkl"
                            fi
