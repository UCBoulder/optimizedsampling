    ...
    for SIZE in 4000; do
        for PPC in 20; do
            if [ "$SIZE" -eq 0 ]; then
                ...
                INIT_SET_IDS="../../0_data/initial_samples/india_secc/cluster_sampling/fixedstrata_01-04-06-08-09-11-21-23-25-33/sample_state_district_${PPC}ppc_${SIZE}_size_seed_1.pkl"
            fi
            for COST_FN in "cluster_based" "uniform"; do
                ...
                for METHOD in "${METHODS[@]}"; do
                    for BUDGET in "${BUDGETS[@]}"; do
                        ...
                        if [ "$COST_FN" == "cluster_based" ]; then
                            UNIT_ASSIGNMENT_PATH="../../0_data/groups/india_secc/district_assignments_dict.pkl"
                            UNIT_TYPE="cluster"
                            POINTS_PER_UNIT=20
                            COST_FN_NAME="region_aware_unit_cost"
                            COST_FN_WITH_SPECIFICS='cluster_based_c1_20_c2_30'
                            REGION_ASSIGNMENT_PATH="../../0_data/groups/india_secc/state_assignments_dict.pkl"
                            IN_REGION_UNIT_COST=20
                            OUT_OF_REGION_UNIT_COST=30
                        fi
