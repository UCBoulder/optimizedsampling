...
run_setting3() {
    ...
    for SIZE in 100; do
        ...
        INIT_SET_IDS="../../0_data/initial_samples/usavars/population/cluster_sampling/fixedstrata_Idaho_16-Louisiana_22-Mississippi_28-New Mexico_35-Pennsylvania_42/sample_state_combined_county_id_10ppc_${SIZE}_size_seed_${seed}.pkl"
                    ...
                    for BUDGET in "${BUDGETS[@]}"; do
                        for UTIL_LAMBDA in "${UTIL_LAMBDAS[@]}"; do
                            ...
                            if [ "$COST_FN" == "cluster_based" ]; then
                                UNIT_ASSIGNMENT_PATH="../../0_data/groups/usavars/population/county_assignments_dict.pkl"
                                UNIT_TYPE="cluster"
                                POINTS_PER_UNIT=10
