...
# CHANGE HERE: Import additional metrics
...
# CHANGE HERE: New function to compute all metrics
...
def main(cfg, train_data):
    ...
    # CHANGE HERE: Use new evaluation function
    X_test, y_test = test_data[:][0], test_data[:][1]

    run_regression = True
    if len(lSet) > 0:
        # CHANGE HERE: Update filename to reflect multiple metrics
        ...
        if os.path.exists(initial_set_metrics_path):
            ...
            print("Loading initial set metrics...")
            try:
                with open(initial_set_metrics_path, "r") as f:
                    metrics = json.load(f)
                # CHANGE HERE: Log all metrics
                logger.info(f"Initial R² score: {metrics['r2']:.4f}")
                logger.info(f"Initial MSE: {metrics['mse']:.4f}")
                logger.info(f"Initial RMSE: {metrics['rmse']:.4f}")
                logger.info(f"Initial MAE: {metrics['mae']:.4f}")
        ...
        if run_regression:
            ...
            if X_train.shape[0] < 5:
                ...
                # CHANGE HERE: Compute all metrics instead of just R2
    ...
    if lSet_updated.size > 0:
        ...
        if n_splits > X_train_updated.shape[0]:
            ...
            # CHANGE HERE: Compute all metrics for updated model
            metrics_updated = evaluate_all_metrics(ridge_search, X_test, y_test)
            logger.info(f"Updated R² score: {metrics_updated['r2']:.4f}")
            logger.info(f"Updated MSE: {metrics_updated['mse']:.4f}")
            logger.info(f"Updated RMSE: {metrics_updated['rmse']:.4f}")
            logger.info(f"Updated MAE: {metrics_updated['mae']:.4f}")
            
            # CHANGE HERE: Save updated metrics
            updated_metrics_path = f"{exp_dir}/updated_set_metrics_seed_{cfg.RNG_SEED}.json"
            with open(updated_metrics_path, "w") as f:
