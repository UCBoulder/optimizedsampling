...
run_setting2() {
    ...
    for SIZE in 100 200 300 400 500 1000; do
        ...
        INIT_SET_IDS="/home/libe2152/optimizedsampling/0_data/initial_samples/usavars/treecover/convenience_sampling/urban_based/IDS_top20_urban_${SIZE}_points_probabilistic_${SIZE}_size_seed_${seed}.pkl"
                ...
                for BUDGET in "${BUDGETS[@]}"; do
                    for UTIL_LAMBDA in "${UTIL_LAMBDAS[@]}"; do
                        ...
                        if [ "$COST_FN" == "convenience_based" ]; then
                            COST_ARRAY_PATH="/home/libe2152/optimizedsampling/0_data/costs/usavars/treecover/convenience_costs/linear_distance_km_based_costs_top20_urban_0.01.pkl"
                            COST_FN_NAME="pointwise_by_array"
                            COST_FN_WITH_SPECIFICS="convenience_based_linear_top20_urban_0.01"
                        fi
                        ...
                        if [ "$METHOD" == "poprisk" ]; then
                            EXP_NAME="usavars_treecover_${INIT_NAME_EXP}_cost_${COST_FN_WITH_SPECIFICS}_method_${METHOD}_${UTIL_LAMBDA}_${GROUP_TYPE}_budget_${BUDGET}_seed_${seed}"
                        fi
