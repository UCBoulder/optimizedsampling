        ...
        for METHOD in "${METHODS[@]}"; do
            for BUDGET in "${BUDGETS[@]}"; do
                ...
                if [ "$COST_FN" == "cluster_based" ]; then
                    ...
                    COST_ARRAY_PATH="/home/libe2152/optimizedsampling/0_data/costs/usavars/population/convenience_costs/distance_based_costs_top50_urban.pkl"
                    COST_FN_NAME="pointwise_by_array"
                    COST_FN_WITH_SPECIFICS='convenience_based_top50_urban'
                fi

                EXP_NAME="usavars_population_${INIT_NAME}_cost_${COST_FN_WITH_SPECIFICS}_method_${METHOD}_budget_${BUDGET}_seed_${seed}"
                if [ "$METHOD" == "poprisk" ]; then
                    EXP_NAME="usavars_population_${INIT_NAME}_cost_${COST_FN_WITH_SPECIFICS}_method_${METHOD}_${UTIL_LAMBDA}_budget_${BUDGET}_seed_${seed}"
                fi
                CMD="python $SCRIPT \
                    --cfg \"$CFG_FILE\" \
                    --exp-name \"$EXP_NAME\" \
