        ...
        for METHOD in "${METHODS[@]}"; do
            for BUDGET in "${BUDGETS[@]}"; do
                ...
                if [ "$COST_FN" == "convenience_based" ]; then
                    COST_ARRAY_PATH="/home/libe2152/optimizedsampling/0_data/costs/togo/convenience_costs/distance_based_costs_top1_urban.pkl"
                    COST_FN_NAME="pointwise_by_array"
                fi

                if [[ "$METHOD" == "poprisk" || "$METHOD" == "match_population_proportion" ]]; then
                    EXP_NAME="togo_${INIT_NAME}_cost_${COST_FN}_method_${METHOD}_${GROUP_TYPE}_budget_${BUDGET}_seed_${seed}"
                else
                    EXP_NAME="togo_${INIT_NAME}_cost_${COST_FN}_method_${METHOD}_budget_${BUDGET}_seed_${seed}"
                fi
                CMD="python $SCRIPT \
                    --cfg \"$CFG_FILE\" \
                    --exp-name \"$EXP_NAME\" \
