...
import mosaiks.code.mosaiks.config as c
from mosaiks.code.mosaiks.utils import io, spatial
...
def geolocate(names):
    latlon = np.zeros((names.__len__(), 2))
    i = 0
    for name in names:
        file_path = f"/share/usavars/uar/{name}"
        with rasterio.open(file_path) as dataset:
            center_pixel_x = dataset.width / 2
            center_pixel_y = dataset.height / 2
            transform_affine = dataset.transform
            centroid_x = transform_affine[0] * center_pixel_x + transform_affine[2]
            centroid_y = transform_affine[4] * center_pixel_y + transform_affine[5]
            # Projected CRS (e.g., UTM)
            projected_crs = dataset.crs
            projected_crs = projected_crs.to_wkt()
            transformer = Transformer.from_crs(projected_crs, "epsg:4326")
    
            # Convert to geographic coordinates (latitude/longitude)
            lat, lon = transformer.transform(centroid_x, centroid_y)
            latlon[i, 0] = lat
            latlon[i, 1] = lon
            i+=1
    return latlon

def get_latlon_from_csv(name):
    tif_file = name
    id_ = tif_file[5:-4]

    label_dfs = pd.read_csv('/share/usavars/population.csv', index_col='ID')

    lat = label_dfs.loc[id_]['lat']
    lon = label_dfs.loc[id_]['lon']
    return [lat, lon]
...
def featurize_and_save(image_folder, out_fpath, c):
    ...
    print("Dill: Features, latlons, and ids")
